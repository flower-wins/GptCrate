...
⚠️ 项目停止维护与替代方案推荐
重要通知：
由于 OpenAI 注册风控机制的频繁更新，本项目目前已失效且不再维护，不再建议尝试运行。
💡 推荐替代方案：
如果您需要稳定、低成本地使用大模型 API，强烈推荐使用：
🔗 Subrouter.ai - 聚合 AI 模型中转站
自动比价：作为聚合平台，它会自动为您调度当前全网价格最便宜、响应最快的底层商家。
省心省力：无需再为注册账号、寻找代理、接码而烦恼，一个 API Key 即可调用所有主流模型。
立即体验：https://subrouter.ai/register?aff=8LPm
